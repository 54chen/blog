---
title: "ZTEZXV10B860AV1.1机顶盒刷机转旁路由手记"
author: "54chen"
...
下面进入正题，如何将ZTEB860AV1.1中兴的电视盒子变废为宝，完成后，达到旁路由效果，全家网络都将通过它达到某种不可在大陆告人的目的。

第一步 花5-10块不等买CH340 USB转TTL连接线一根
---------------------------------------------

正二八经刷机专用的，基本上这些盒子也好、路由器也好，都支持，一般TTL的接线就三条主要的：GND地线，RXD收数据，TXD发数据。很少需要给板子供电才能玩的(某些开源硬件需要)。
...
所有家里要使用的设备，只要修改原来的网关192.168.1.1为192.168.1.10即可（wifi还是连原来的）,然后就通过这个旁路由拥有了它的网络能力。

到此，旁路由功能完成。

以上所有用到的文件，包括名为MstarUpgrade.bin、xxx-linux-arm.tar.gz、config.json 有尝提供:) 联系邮件 czhttp@gmail.com。
